class Disassembler:
    def disassemble(self, memory, address):
        opcode = memory[address]
        method, operation, size = self.opcodes[opcode]
        if method:
            return method(self, memory, address, operation, size)
        if opcode == 0xCB:
            method, operation, size = self.after_CB[memory[(address + 1) % 65536]]
        elif opcode == 0xED:
            method, operation, size = self.after_ED[memory[(address + 1) % 65536]]
        elif opcode == 0xDD:
            method, operation, size = self.after_DD[memory[(address + 1) % 65536]]
            if method is None:
                method, operation, size = self.after_DDCB[memory[(address + 3) % 65536]]
        else:
            method, operation, size = self.after_FD[memory[(address + 1) % 65536]]
            if method is None:
                method, operation, size = self.after_FDCB[memory[(address + 3) % 65536]]
        return method(self, memory, address, operation, size)

    def operation(self, memory, address, operation, size):
        return operation, size

    def byte(self, memory, address, operation, size):
        value = memory[(address + size - 1) % 65536]
        return operation.format(f'${value:02X}'), size

    def word(self, memory, address, operation, size):
        value = memory[(address + size - 2) % 65536] + 256 * memory[(address + size - 1) % 65536]
        return operation.format(f'${value:04X}'), size

    def jump_offset(self, memory, address, operation, size):
    ...
    def defb(self, memory, address, operation, size):
        if size == 1:
