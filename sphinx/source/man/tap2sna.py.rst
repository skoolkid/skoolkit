...
* ``accelerator`` - the tape-sampling loop accelerator to use (default:
  ...
  tape-sampling loop; use ``accelerator=list`` to list the accelerators used
  during a simulated LOAD, along with the hit/miss counts generated by the
  tape-sampling loop detector
