...
* ``accelerate-dec-a`` - enable acceleration of 'DEC A: JR NZ,$-1' delay loops
  ...
  (``0``)
* ``accelerator`` - the tape-sampling loop accelerator to use (see below)
...
The ``accelerator`` parameter must be either a specific accelerator name or one
of the following special values:

* ``auto`` - select an accelerator automatically (this is the default)
