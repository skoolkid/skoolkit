...
Control directive post-processors
---------------------------------
These objects are reponsible for modifying a set of control directives before
:ref:`sna2ctl.py` writes the corresponding control file. They are not used by
default, but may be activated by the ``--process-ctls`` option.

A control directive post-processor must supply the following API method, in
common with skoolkit.rst.RSTHandler:

.. automethod:: skoolkit.rst.RSTHandler.process_ctls

If the post-processor is a class, then its constructor will be called with a
configuration object as the sole argument, as is the case for
skoolkit.rst.RSTHandler:

.. autoclass:: skoolkit.rst.RSTHandler
   :exclude-members: process_ctls
   :noindex:

The keys of the *ctls* dictionary are addresses, and the values are block-level
control directives ('b', 'c', 'g', 'i', 's', 't', 'u' or 'w'). The keys of the
*subctls* dictionary are also addresses, but the values are 4-element lists of
the form::

  [subctl, length, sublengths, comment]

where:

* ``subctl`` is the sub-block control directive ('B', 'C', 'S', 'T' or 'W')
* ``length`` is the sub-block length
* ``sublengths`` is a sequence of sublengths for the sub-block (which may be
  empty or *None*)
* ``comment`` is the sub-block comment (which may be an empty string or *None*)

By default, only one control directive post-processor is specified:
skoolkit.rst.RSTHandler. Its configuration is read from the
``[skoolkit.rst.RSTHandler]`` section of *skoolkit.ini*. The recognised
configuration parameters this section may contain and their default values are:

* ``RST00`` - handler for 'RST $00' instructions (none)
* ``RST08`` - handler for 'RST $08' instructions ('DEFB' - see below)
* ``RST10`` - handler for 'RST $10' instructions (none)
* ``RST18`` - handler for 'RST $18' instructions (none)
* ``RST20`` - handler for 'RST $20' instructions (none)
* ``RST28`` - handler for 'RST $28' instructions (none)
* ``RST30`` - handler for 'RST $30' instructions (none)
* ``RST38`` - handler for 'RST $38' instructions (none)

Valid values for these configuration parameters are:

* ``DEFB`` - add a 'B' directive at the address after the RST instruction
* ``DEFW`` - add a 'W' directive at the address after the RST instruction

To specify two or more control directive post-processors, separate their names
with semicolons. For example::

  CtlPostProcessors=skoolkit.rst.RSTHandler;mods:custom.CtlPostProcessor

This specifies a second post-processor: CtlPostProcessor in *mods/custom.py*.
When used, its configuration will be read from the
``[mods:custom.CtlPostProcessor]`` section of *skoolkit.ini*, if present.

When the :meth:`process_ctls` method is called on the first post-processor,
*ctls* is the dictionary of control directives generated by *sna2ctl.py*, and
the *subctls* dictionary is empty. That post-processor may then modify *ctls*
or *subctls* before they are passed to the second post-processor.
