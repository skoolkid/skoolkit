...
class HtmlWriter:
    ...
    def expand_audio(self, text, index, cwd):
        end, interrupts, contention, offset, beeper, volume, fname, delays, audio_log = skoolmacro.parse_audio(self, text, index, self._need_audio)
        if delays or audio_log:
            with self.file_info.open_file(fname, mode='wb') as f:
                if delays:
                    ...
                    options = Options(volume, beeper=beeper)
