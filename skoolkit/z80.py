...
def get_timing(instruction):
    if not instruction.operation.upper().startswith('DEF') and instruction.bytes:
        ...
        if opcode == 0xCB:
            return Simulator.after_CB[instruction.bytes[1]][0]
        if opcode == 0xED:
            return Simulator.after_ED[instruction.bytes[1]][0]
        if opcode in (0xDD, 0xFD):
            opcode2 = instruction.bytes[1]
            if opcode2 == 0xCB:
                return Simulator.after_DDCB[instruction.bytes[3]][0]
            return Simulator.after_DD[opcode2][0]
        return Simulator.opcodes[opcode][0]
