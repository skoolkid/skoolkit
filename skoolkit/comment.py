...
class CommentGenerator:
    def __init__(self):
        self.ops = {
            ...
            0x22: (self.addr_arg, LD_mm_rr.format('l', 'h')),
            ...
            0x2A: (self.addr_arg, LD_rr_mm.format('l', 'h')),
            ...
            0x32: (self.word_arg, LD.format(f'({WORD})', '#REGa')),
            ...
            0x3A: (self.word_arg, LD.format('#REGa', f'({WORD})')),
        ...
        self.after_DD = {
            ...
            0x22: (self.addr_arg, LD_mm_rr.format('ixl', 'ixh')),
            ...
            0x66: (self.index, LD.format('#REGh', '{ixd}')),
        ...
        self.after_ED = {
            ...
            0x73: (self.addr_arg, LD_mm_rr.format('sp-lo', 'sp-hi')),
            0x7A: (None, ADC.format('#REGhl', '#REGsp')),
            0x7B: (self.addr_arg, LD.format('#REGsp', '({})+#N(256,2,,1)($)*({})')),
            0xA0: (None, '(#REGde)=(#REGhl), #REGhl+=1, #REGde+=1, #REGbc-=1'),
            0xA1: (None, 'Compare #REGa with (#REGhl), #REGhl+=1, #REGbc-=1'),
            0xA2: (None, 'Read from port #REGbc into (#REGhl), #REGhl+=1, #REGb-=1'),
            0xA3: (None, '#REGb-=1, output (#REGhl) to port #REGbc, #REGhl+=1'),
            0xA8: (None, '(#REGde)=(#REGhl), #REGhl-=1, #REGde-=1, #REGbc-=1'),
            0xA9: (None, 'Compare #REGa with (#REGhl), #REGhl-=1, #REGbc-=1'),
            0xAA: (None, 'Read from port #REGbc into (#REGhl), #REGhl-=1, #REGb-=1'),
            0xAB: (None, '#REGb-=1, output (#REGhl) to port #REGbc, #REGhl-=1'),
            0xB0: (None, '(#REGde)=(#REGhl), #REGhl+=1, #REGde+=1, #REGbc-=1; repeat until #REGbc=0'),
            0xB1: (None, 'Compare #REGa with (#REGhl), #REGhl+=1, #REGbc-=1; repeat until #REGbc=0 or #REGa=(#REGhl)'),
            0xB2: (None, 'Read from port #REGbc into (#REGhl), #REGhl+=1, #REGb-=1; repeat until #REGb=0'),
            0xB3: (None, '#REGb-=1, output (#REGhl) to port #REGbc, #REGhl+=1; repeat until #REGb=0'),
            0xB8: (None, '(#REGde)=(#REGhl), #REGhl-=1, #REGde-=1, #REGbc-=1; repeat until #REGbc=0'),
            0xB9: (None, 'Compare #REGa with (#REGhl), #REGhl-=1, #REGbc-=1; repeat until #REGbc=0 or #REGa=(#REGhl)'),
            0xBA: (None, 'Read from port #REGbc into (#REGhl), #REGhl-=1, #REGb-=1; repeat until #REGb=0'),
            0xBB: (None, '#REGb-=1, output (#REGhl) to port #REGbc, #REGhl-=1; repeat until #REGb=0')
        }
        ...
        for i, r in enumerate(REGISTERS):
            ...
            self.ops[0x80 + i] = (None, ADD_A.format(r))
            self.ops[0x88 + i] = (None, ADC_A.format(r))
            if i < 7:
                self.ops[0x90 + i] = (None, SUB.format(r))
                self.ops[0x98 + i] = (None, SBC_A.format(r))
                self.ops[0xA0 + i] = (None, AND.format(r))
                self.ops[0xA8 + i] = (None, XOR.format(r))
                self.ops[0xB0 + i] = (None, OR.format(r))
                self.ops[0xB8 + i] = (None, CP.format(r))
            self.ops[0xC7 + 8 * i] = (None, RST.format(8 * i))
            if i != 6:
                self.after_DD[0x70 + i] = (self.index, f'{{ixd}}={r}')
    ...
    def addr_arg(self, template, address, values):
        ...
        return template.format(f'#N({addr},4,,1)($)', f'#N({addr + 1},4,,1)($)')
