...
class CMIOSimulator(Simulator):
    ...
    def halt(self, registers):
        ...
        def func():
            ...
            if t0 < registers[25] % frame_duration < t1:
                ...
                delay = 0
            registers[25] += 4 + delay # T-states
            if registers[26] and registers[25] % frame_duration < self.int_active:
                registers[24] = (pc + 1) % 65536 # PC
    ...
    def in_a(self, registers, memory):
        # IN A,(n)
        t0, t1, frame_duration, contend, io_contention = self.t0, self.t1, self.frame_duration, self.contend, self.io_contention
        def func():
            pc = registers[24]
            pc1 = (pc + 1) % 65536
