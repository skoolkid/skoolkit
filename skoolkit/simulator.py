...
class Simulator:
    ...
    def add_a(self, registers, memory, timing, size, reg=N, carry=0, mult=1):
        ...
        if mult == 1:
            s_value = value
            if (carry and reg == 0 and old_a % 16 == 0x0F) or ((old_a % 16) + (addend % 16)) & 0x10:
                f += 0x10 # ...H....
        else:
            s_value = ~value # Flip sign bit when subtracting
            if carry and reg == 0 and old_a % 16 == 0x0F:
                f += old_c * 16 # ...H....
    ...
    def add16(self, registers, timing, size, augend, reg, carry=0, mult=1):
        ...
        registers[1] = f + ((value // 256) & 0x28) # ..5.3...
    ...
    def bit(self, registers, memory, timing, size, mask, reg):
        ...
        if reg >= 32:
            ...
            if reg == 32:
                ...
                v = (registers[11] + 256 * registers[10] + offset) % 65536
    ...
    def dec_r(self, registers, reg):
        ...
        f = (registers[1] % 2) + (value & 0xA8) + 0x02 # S.5.3.NC
        if value == 0x7F:
            f += 0x14 # ...H.P..
        elif value % 16 == 0x0F:
            f += 0x10 # ...H....
    ...
    def inc_r(self, registers, reg):
        value = (registers[reg] + 1) % 256
        f = (registers[1] % 2) + (value & 0xA8) # S.5.3..C
        if value == 0x80:
            f += 0x14 # ...H.P..
        elif value % 16 == 0x00:
            f += 0x10 # ...H....
    ...
    def rotate_a(self, registers, cbit, carry=0):
        ...
        if carry:
            ...
            # RRA
            value = ((old_f * 128) + (a // 2)) % 256
        else:
            # RLA
            value = ((old_f % 2) + (a * 2)) % 256
        registers[0] = value
        if a & cbit:
    ...
    def rotate_r(self, registers, cbit, reg, carry=0):
        ...
        if carry:
            ...
            # RR r
            value = ((registers[1] * 128) + (r // 2)) % 256
        else:
            # RL r
            value = ((registers[1] % 2) + (r * 2)) % 256
        registers[reg] = value
        if r & cbit:
    ...
    def create_opcodes(self):
        ...
        self.opcodes = [
            ...
            partial(self.rotate_a, r, 128, 1),                 # 0x07 RLCA
            ...
            partial(self.rotate_a, r, 1, 1),                   # 0x0F RRCA
            ...
            partial(self.rotate_a, r, 128),                    # 0x17 RLA
            ...
            partial(self.rotate_a, r, 1),                      # 0x1F RRA
            partial(self.jr, r, m, 64, 64),                    # 0x20 JR NZ,nn
            ...
            partial(self.jr, r, m, 64, 0),                     # 0x28 JR Z,nn
            ...
            partial(self.jr, r, m, 1, 1),                      # 0x30 JR NC,nn
            ...
            partial(self.jr, r, m, 1, 0),                      # 0x38 JR C,nn
            ...
            partial(self.add_a, r, m, 4, 1, B, 1),             # 0x88 ADC A,B
            partial(self.add_a, r, m, 4, 1, C, 1),             # 0x89 ADC A,C
            partial(self.add_a, r, m, 4, 1, D, 1),             # 0x8A ADC A,D
            partial(self.add_a, r, m, 4, 1, E, 1),             # 0x8B ADC A,E
            partial(self.add_a, r, m, 4, 1, H, 1),             # 0x8C ADC A,H
            partial(self.add_a, r, m, 4, 1, L, 1),             # 0x8D ADC A,L
            partial(self.add_a, r, m, 7, 1, Hd, 1),            # 0x8E ADC A,(HL)
            partial(self.add_a, r, m, 4, 1, A, 1),             # 0x8F ADC A,A
            partial(self.add_a, r, m, 4, 1, B, 0, -1),         # 0x90 SUB B
            partial(self.add_a, r, m, 4, 1, C, 0, -1),         # 0x91 SUB C
            partial(self.add_a, r, m, 4, 1, D, 0, -1),         # 0x92 SUB D
            partial(self.add_a, r, m, 4, 1, E, 0, -1),         # 0x93 SUB E
            partial(self.add_a, r, m, 4, 1, H, 0, -1),         # 0x94 SUB H
            partial(self.add_a, r, m, 4, 1, L, 0, -1),         # 0x95 SUB L
            partial(self.add_a, r, m, 7, 1, Hd, 0, -1),        # 0x96 SUB (HL)
            partial(self.add_a, r, m, 4, 1, A, 0, -1),         # 0x97 SUB A
            partial(self.add_a, r, m, 4, 1, B, 1, -1),         # 0x98 SBC A,B
            partial(self.add_a, r, m, 4, 1, C, 1, -1),         # 0x99 SBC A,C
            partial(self.add_a, r, m, 4, 1, D, 1, -1),         # 0x9A SBC A,D
            partial(self.add_a, r, m, 4, 1, E, 1, -1),         # 0x9B SBC A,E
            partial(self.add_a, r, m, 4, 1, H, 1, -1),         # 0x9C SBC A,H
            partial(self.add_a, r, m, 4, 1, L, 1, -1),         # 0x9D SBC A,L
            partial(self.add_a, r, m, 7, 1, Hd, 1, -1),        # 0x9E SBC A,(HL)
            partial(self.add_a, r, m, 4, 1, A, 1, -1),         # 0x9F SBC A,A
            ...
            partial(self.add_a, r, m, 7, 2),                   # 0xC6 ADD A,n
            ...
            partial(self.add_a, r, m, 7, 2, N, 1, 1),          # 0xCE ADC A,n
            ...
            partial(self.add_a, r, m, 7, 2, N, 0, -1),         # 0xD6 SUB n
            ...
            None,                                              # 0xDD DD prefix
            partial(self.add_a, r, m, 7, 2, N, 1, -1),         # 0xDE SBC A,n
        ...
        self.after_CB = [
            partial(self.rotate_r, r, 128, B, 1),              # 0x00 RLC B
            partial(self.rotate_r, r, 128, C, 1),              # 0x01 RLC C
            partial(self.rotate_r, r, 128, D, 1),              # 0x02 RLC D
            partial(self.rotate_r, r, 128, E, 1),              # 0x03 RLC E
            partial(self.rotate_r, r, 128, H, 1),              # 0x04 RLC H
            partial(self.rotate_r, r, 128, L, 1),              # 0x05 RLC L
            partial(self.rotate, r, m, 15, 2, 128, Hd, 1),     # 0x06 RLC (HL)
            partial(self.rotate_r, r, 128, A, 1),              # 0x07 RLC A
            partial(self.rotate_r, r, 1, B, 1),                # 0x08 RRC B
            partial(self.rotate_r, r, 1, C, 1),                # 0x09 RRC C
            partial(self.rotate_r, r, 1, D, 1),                # 0x0A RRC D
            partial(self.rotate_r, r, 1, E, 1),                # 0x0B RRC E
            partial(self.rotate_r, r, 1, H, 1),                # 0x0C RRC H
            partial(self.rotate_r, r, 1, L, 1),                # 0x0D RRC L
            partial(self.rotate, r, m, 15, 2, 1, Hd, 1),       # 0x0E RRC (HL)
            partial(self.rotate_r, r, 1, A, 1),                # 0x0F RRC A
            partial(self.rotate_r, r, 128, B),                 # 0x10 RL B
            partial(self.rotate_r, r, 128, C),                 # 0x11 RL C
            partial(self.rotate_r, r, 128, D),                 # 0x12 RL D
            partial(self.rotate_r, r, 128, E),                 # 0x13 RL E
            partial(self.rotate_r, r, 128, H),                 # 0x14 RL H
            partial(self.rotate_r, r, 128, L),                 # 0x15 RL L
            partial(self.rotate, r, m, 15, 2, 128, Hd),        # 0x16 RL (HL)
            partial(self.rotate_r, r, 128, A),                 # 0x17 RL A
            partial(self.rotate_r, r, 1, B),                   # 0x18 RR B
            partial(self.rotate_r, r, 1, C),                   # 0x19 RR C
            partial(self.rotate_r, r, 1, D),                   # 0x1A RR D
            partial(self.rotate_r, r, 1, E),                   # 0x1B RR E
            partial(self.rotate_r, r, 1, H),                   # 0x1C RR H
            partial(self.rotate_r, r, 1, L),                   # 0x1D RR L
            partial(self.rotate, r, m, 15, 2, 1, Hd),          # 0x1E RR (HL)
            partial(self.rotate_r, r, 1, A),                   # 0x1F RR A
            partial(self.shift, r, m, 8, 2, 0, 128, B),        # 0x20 SLA B
            partial(self.shift, r, m, 8, 2, 0, 128, C),        # 0x21 SLA C
            partial(self.shift, r, m, 8, 2, 0, 128, D),        # 0x22 SLA D
            partial(self.shift, r, m, 8, 2, 0, 128, E),        # 0x23 SLA E
            partial(self.shift, r, m, 8, 2, 0, 128, H),        # 0x24 SLA H
            partial(self.shift, r, m, 8, 2, 0, 128, L),        # 0x25 SLA L
            partial(self.shift, r, m, 15, 2, 0, 128, Hd),      # 0x26 SLA (HL)
            partial(self.shift, r, m, 8, 2, 0, 128, A),        # 0x27 SLA A
            partial(self.shift, r, m, 8, 2, 1, 1, B),          # 0x28 SRA B
            partial(self.shift, r, m, 8, 2, 1, 1, C),          # 0x29 SRA C
            partial(self.shift, r, m, 8, 2, 1, 1, D),          # 0x2A SRA D
            partial(self.shift, r, m, 8, 2, 1, 1, E),          # 0x2B SRA E
            partial(self.shift, r, m, 8, 2, 1, 1, H),          # 0x2C SRA H
            partial(self.shift, r, m, 8, 2, 1, 1, L),          # 0x2D SRA L
            partial(self.shift, r, m, 15, 2, 1, 1, Hd),        # 0x2E SRA (HL)
            partial(self.shift, r, m, 8, 2, 1, 1, A),          # 0x2F SRA A
            partial(self.shift, r, m, 8, 2, 2, 128, B),        # 0x30 SLL B
            partial(self.shift, r, m, 8, 2, 2, 128, C),        # 0x31 SLL C
            partial(self.shift, r, m, 8, 2, 2, 128, D),        # 0x32 SLL D
            partial(self.shift, r, m, 8, 2, 2, 128, E),        # 0x33 SLL E
            partial(self.shift, r, m, 8, 2, 2, 128, H),        # 0x34 SLL H
            partial(self.shift, r, m, 8, 2, 2, 128, L),        # 0x35 SLL L
            partial(self.shift, r, m, 15, 2, 2, 128, Hd),      # 0x36 SLL (HL)
            partial(self.shift, r, m, 8, 2, 2, 128, A),        # 0x37 SLL A
            partial(self.shift, r, m, 8, 2, 3, 1, B),          # 0x38 SRL B
            partial(self.shift, r, m, 8, 2, 3, 1, C),          # 0x39 SRL C
            partial(self.shift, r, m, 8, 2, 3, 1, D),          # 0x3A SRL D
            partial(self.shift, r, m, 8, 2, 3, 1, E),          # 0x3B SRL E
            partial(self.shift, r, m, 8, 2, 3, 1, H),          # 0x3C SRL H
            partial(self.shift, r, m, 8, 2, 3, 1, L),          # 0x3D SRL L
            partial(self.shift, r, m, 15, 2, 3, 1, Hd),        # 0x3E SRL (HL)
            partial(self.shift, r, m, 8, 2, 3, 1, A),          # 0x3F SRL A
        ...
        self.after_ED = [
            ...
            partial(self.add16, r, 15, 2, H, B, 1),            # 0x4A ADC HL,BC
            ...
            partial(self.add16, r, 15, 2, H, D, 1, -1),        # 0x52 SBC HL,DE
            ...
            partial(self.add16, r, 15, 2, H, D, 1),            # 0x5A ADC HL,DE
            ...
            partial(self.add16, r, 15, 2, H, H, 1, -1),        # 0x62 SBC HL,HL
            ...
            partial(self.add16, r, 15, 2, H, H, 1),            # 0x6A ADC HL,HL
            ...
            partial(self.add16, r, 15, 2, H, SP, 1, -1),       # 0x72 SBC HL,SP
        ...
        self.after_DD = [
            ...
            partial(self.add_a, r, m, 8, 2, IXh, 1),           # 0x8C ADC A,IXh
            partial(self.add_a, r, m, 8, 2, IXl, 1),           # 0x8D ADC A,IXl
            partial(self.add_a, r, m, 19, 3, Xd, 1),           # 0x8E ADC A,(IX+d)
            ...
            partial(self.add_a, r, m, 8, 2, IXh, 0, -1),       # 0x94 SUB IXh
            partial(self.add_a, r, m, 8, 2, IXl, 0, -1),       # 0x95 SUB IXl
            partial(self.add_a, r, m, 19, 3, Xd, 0, -1),       # 0x96 SUB (IX+d)
            ...
            partial(self.add_a, r, m, 8, 2, IXh, 1, -1),       # 0x9C SBC A,IXh
            partial(self.add_a, r, m, 8, 2, IXl, 1, -1),       # 0x9D SBC A,IXl
            partial(self.add_a, r, m, 19, 3, Xd, 1, -1),       # 0x9E SBC A,(IX+d)
        ...
        self.after_DDCB = [
            partial(self.rotate, r, m, 23, 4, 128, Xd, 1, B),  # 0x00 RLC (IX+d),B
            partial(self.rotate, r, m, 23, 4, 128, Xd, 1, C),  # 0x01 RLC (IX+d),C
            partial(self.rotate, r, m, 23, 4, 128, Xd, 1, D),  # 0x02 RLC (IX+d),D
            partial(self.rotate, r, m, 23, 4, 128, Xd, 1, E),  # 0x03 RLC (IX+d),E
            partial(self.rotate, r, m, 23, 4, 128, Xd, 1, H),  # 0x04 RLC (IX+d),H
            partial(self.rotate, r, m, 23, 4, 128, Xd, 1, L),  # 0x05 RLC (IX+d),L
            partial(self.rotate, r, m, 23, 4, 128, Xd, 1),     # 0x06 RLC (IX+d)
            partial(self.rotate, r, m, 23, 4, 128, Xd, 1, A),  # 0x07 RLC (IX+d),A
            partial(self.rotate, r, m, 23, 4, 1, Xd, 1, B),    # 0x08 RRC (IX+d),B
            partial(self.rotate, r, m, 23, 4, 1, Xd, 1, C),    # 0x09 RRC (IX+d),C
            partial(self.rotate, r, m, 23, 4, 1, Xd, 1, D),    # 0x0A RRC (IX+d),D
            partial(self.rotate, r, m, 23, 4, 1, Xd, 1, E),    # 0x0B RRC (IX+d),E
            partial(self.rotate, r, m, 23, 4, 1, Xd, 1, H),    # 0x0C RRC (IX+d),H
            partial(self.rotate, r, m, 23, 4, 1, Xd, 1, L),    # 0x0D RRC (IX+d),L
            partial(self.rotate, r, m, 23, 4, 1, Xd, 1),       # 0x0E RRC (IX+d)
            partial(self.rotate, r, m, 23, 4, 1, Xd, 1, A),    # 0x0F RRC (IX+d),A
            partial(self.rotate, r, m, 23, 4, 128, Xd, 0, B),  # 0x10 RL (IX+d),B
            partial(self.rotate, r, m, 23, 4, 128, Xd, 0, C),  # 0x11 RL (IX+d),C
            partial(self.rotate, r, m, 23, 4, 128, Xd, 0, D),  # 0x12 RL (IX+d),D
            partial(self.rotate, r, m, 23, 4, 128, Xd, 0, E),  # 0x13 RL (IX+d),E
            partial(self.rotate, r, m, 23, 4, 128, Xd, 0, H),  # 0x14 RL (IX+d),H
            partial(self.rotate, r, m, 23, 4, 128, Xd, 0, L),  # 0x15 RL (IX+d),L
            partial(self.rotate, r, m, 23, 4, 128, Xd),        # 0x16 RL (IX+d)
            partial(self.rotate, r, m, 23, 4, 128, Xd, 0, A),  # 0x17 RL (IX+d),A
            partial(self.rotate, r, m, 23, 4, 1, Xd, 0, B),    # 0x18 RR (IX+d),B
            partial(self.rotate, r, m, 23, 4, 1, Xd, 0, C),    # 0x19 RR (IX+d),C
            partial(self.rotate, r, m, 23, 4, 1, Xd, 0, D),    # 0x1A RR (IX+d),D
            partial(self.rotate, r, m, 23, 4, 1, Xd, 0, E),    # 0x1B RR (IX+d),E
            partial(self.rotate, r, m, 23, 4, 1, Xd, 0, H),    # 0x1C RR (IX+d),H
            partial(self.rotate, r, m, 23, 4, 1, Xd, 0, L),    # 0x1D RR (IX+d),L
            partial(self.rotate, r, m, 23, 4, 1, Xd),          # 0x1E RR (IX+d)
            partial(self.rotate, r, m, 23, 4, 1, Xd, 0, A),    # 0x1F RR (IX+d),A
            partial(self.shift, r, m, 23, 4, 0, 128, Xd, B),   # 0x20 SLA (IX+d),B
            partial(self.shift, r, m, 23, 4, 0, 128, Xd, C),   # 0x21 SLA (IX+d),C
            partial(self.shift, r, m, 23, 4, 0, 128, Xd, D),   # 0x22 SLA (IX+d),D
            partial(self.shift, r, m, 23, 4, 0, 128, Xd, E),   # 0x23 SLA (IX+d),E
            partial(self.shift, r, m, 23, 4, 0, 128, Xd, H),   # 0x24 SLA (IX+d),H
            partial(self.shift, r, m, 23, 4, 0, 128, Xd, L),   # 0x25 SLA (IX+d),L
            partial(self.shift, r, m, 23, 4, 0, 128, Xd),      # 0x26 SLA (IX+d)
            partial(self.shift, r, m, 23, 4, 0, 128, Xd, A),   # 0x27 SLA (IX+d),A
            partial(self.shift, r, m, 23, 4, 1, 1, Xd, B),     # 0x28 SRA (IX+d),B
            partial(self.shift, r, m, 23, 4, 1, 1, Xd, C),     # 0x29 SRA (IX+d),C
            partial(self.shift, r, m, 23, 4, 1, 1, Xd, D),     # 0x2A SRA (IX+d),D
            partial(self.shift, r, m, 23, 4, 1, 1, Xd, E),     # 0x2B SRA (IX+d),E
            partial(self.shift, r, m, 23, 4, 1, 1, Xd, H),     # 0x2C SRA (IX+d),H
            partial(self.shift, r, m, 23, 4, 1, 1, Xd, L),     # 0x2D SRA (IX+d),L
            partial(self.shift, r, m, 23, 4, 1, 1, Xd),        # 0x2E SRA (IX+d)
            partial(self.shift, r, m, 23, 4, 1, 1, Xd, A),     # 0x2F SRA (IX+d),A
            partial(self.shift, r, m, 23, 4, 2, 128, Xd, B),   # 0x30 SLL (IX+d),B
            partial(self.shift, r, m, 23, 4, 2, 128, Xd, C),   # 0x31 SLL (IX+d),C
            partial(self.shift, r, m, 23, 4, 2, 128, Xd, D),   # 0x32 SLL (IX+d),D
            partial(self.shift, r, m, 23, 4, 2, 128, Xd, E),   # 0x33 SLL (IX+d),E
            partial(self.shift, r, m, 23, 4, 2, 128, Xd, H),   # 0x34 SLL (IX+d),H
            partial(self.shift, r, m, 23, 4, 2, 128, Xd, L),   # 0x35 SLL (IX+d),L
            partial(self.shift, r, m, 23, 4, 2, 128, Xd),      # 0x36 SLL (IX+d)
            partial(self.shift, r, m, 23, 4, 2, 128, Xd, A),   # 0x37 SLL (IX+d),A
            partial(self.shift, r, m, 23, 4, 3, 1, Xd, B),     # 0x38 SRL (IX+d),B
            partial(self.shift, r, m, 23, 4, 3, 1, Xd, C),     # 0x39 SRL (IX+d),C
            partial(self.shift, r, m, 23, 4, 3, 1, Xd, D),     # 0x3A SRL (IX+d),D
            partial(self.shift, r, m, 23, 4, 3, 1, Xd, E),     # 0x3B SRL (IX+d),E
            partial(self.shift, r, m, 23, 4, 3, 1, Xd, H),     # 0x3C SRL (IX+d),H
            partial(self.shift, r, m, 23, 4, 3, 1, Xd, L),     # 0x3D SRL (IX+d),L
            partial(self.shift, r, m, 23, 4, 3, 1, Xd),        # 0x3E SRL (IX+d)
            partial(self.shift, r, m, 23, 4, 3, 1, Xd, A),     # 0x3F SRL (IX+d),A
        ...
        self.after_FD = [
            ...
            partial(self.nop_dd_fd, r),                        # 0x8A
            partial(self.nop_dd_fd, r),                        # 0x8B
            partial(self.add_a, r, m, 8, 2, IYh, 1),           # 0x8C ADC A,IYh
            partial(self.add_a, r, m, 8, 2, IYl, 1),           # 0x8D ADC A,IYl
            partial(self.add_a, r, m, 19, 3, Yd, 1),           # 0x8E ADC A,(IY+d)
            partial(self.nop_dd_fd, r),                        # 0x8F
            partial(self.nop_dd_fd, r),                        # 0x90
            partial(self.nop_dd_fd, r),                        # 0x91
            partial(self.nop_dd_fd, r),                        # 0x92
            partial(self.nop_dd_fd, r),                        # 0x93
            partial(self.add_a, r, m, 8, 2, IYh, 0, -1),       # 0x94 SUB IYh
            partial(self.add_a, r, m, 8, 2, IYl, 0, -1),       # 0x95 SUB IYl
            partial(self.add_a, r, m, 19, 3, Yd, 0, -1),       # 0x96 SUB (IY+d)
            partial(self.nop_dd_fd, r),                        # 0x97
            partial(self.nop_dd_fd, r),                        # 0x98
            partial(self.nop_dd_fd, r),                        # 0x99
            partial(self.nop_dd_fd, r),                        # 0x9A
            partial(self.nop_dd_fd, r),                        # 0x9B
            partial(self.add_a, r, m, 8, 2, IYh, 1, -1),       # 0x9C SBC A,IYh
            partial(self.add_a, r, m, 8, 2, IYl, 1, -1),       # 0x9D SBC A,IYl
            partial(self.add_a, r, m, 19, 3, Yd, 1, -1),       # 0x9E SBC A,(IY+d)
            partial(self.nop_dd_fd, r),                        # 0x9F
            partial(self.nop_dd_fd, r),                        # 0xA0
            partial(self.nop_dd_fd, r),                        # 0xA1
            partial(self.nop_dd_fd, r),                        # 0xA2
        ...
        self.after_FDCB = [
            partial(self.rotate, r, m, 23, 4, 128, Yd, 1, B),  # 0x00 RLC (IY+d),B
            partial(self.rotate, r, m, 23, 4, 128, Yd, 1, C),  # 0x01 RLC (IY+d),C
            partial(self.rotate, r, m, 23, 4, 128, Yd, 1, D),  # 0x02 RLC (IY+d),D
            partial(self.rotate, r, m, 23, 4, 128, Yd, 1, E),  # 0x03 RLC (IY+d),E
            partial(self.rotate, r, m, 23, 4, 128, Yd, 1, H),  # 0x04 RLC (IY+d),H
            partial(self.rotate, r, m, 23, 4, 128, Yd, 1, L),  # 0x05 RLC (IY+d),L
            partial(self.rotate, r, m, 23, 4, 128, Yd, 1),     # 0x06 RLC (IY+d)
            partial(self.rotate, r, m, 23, 4, 128, Yd, 1, A),  # 0x07 RLC (IY+d),A
            partial(self.rotate, r, m, 23, 4, 1, Yd, 1, B),    # 0x08 RRC (IY+d),B
            partial(self.rotate, r, m, 23, 4, 1, Yd, 1, C),    # 0x09 RRC (IY+d),C
            partial(self.rotate, r, m, 23, 4, 1, Yd, 1, D),    # 0x0A RRC (IY+d),D
            partial(self.rotate, r, m, 23, 4, 1, Yd, 1, E),    # 0x0B RRC (IY+d),E
            partial(self.rotate, r, m, 23, 4, 1, Yd, 1, H),    # 0x0C RRC (IY+d),H
            partial(self.rotate, r, m, 23, 4, 1, Yd, 1, L),    # 0x0D RRC (IY+d),L
            partial(self.rotate, r, m, 23, 4, 1, Yd, 1),       # 0x0E RRC (IY+d)
            partial(self.rotate, r, m, 23, 4, 1, Yd, 1, A),    # 0x0F RRC (IY+d),A
            partial(self.rotate, r, m, 23, 4, 128, Yd, 0, B),  # 0x10 RL (IY+d),B
            partial(self.rotate, r, m, 23, 4, 128, Yd, 0, C),  # 0x11 RL (IY+d),C
            partial(self.rotate, r, m, 23, 4, 128, Yd, 0, D),  # 0x12 RL (IY+d),D
            partial(self.rotate, r, m, 23, 4, 128, Yd, 0, E),  # 0x13 RL (IY+d),E
            partial(self.rotate, r, m, 23, 4, 128, Yd, 0, H),  # 0x14 RL (IY+d),H
            partial(self.rotate, r, m, 23, 4, 128, Yd, 0, L),  # 0x15 RL (IY+d),L
            partial(self.rotate, r, m, 23, 4, 128, Yd),        # 0x16 RL (IY+d)
            partial(self.rotate, r, m, 23, 4, 128, Yd, 0, A),  # 0x17 RL (IY+d),A
            partial(self.rotate, r, m, 23, 4, 1, Yd, 0, B),    # 0x18 RR (IY+d),B
            partial(self.rotate, r, m, 23, 4, 1, Yd, 0, C),    # 0x19 RR (IY+d),C
            partial(self.rotate, r, m, 23, 4, 1, Yd, 0, D),    # 0x1A RR (IY+d),D
            partial(self.rotate, r, m, 23, 4, 1, Yd, 0, E),    # 0x1B RR (IY+d),E
            partial(self.rotate, r, m, 23, 4, 1, Yd, 0, H),    # 0x1C RR (IY+d),H
            partial(self.rotate, r, m, 23, 4, 1, Yd, 0, L),    # 0x1D RR (IY+d),L
            partial(self.rotate, r, m, 23, 4, 1, Yd),          # 0x1E RR (IY+d)
            partial(self.rotate, r, m, 23, 4, 1, Yd, 0, A),    # 0x1F RR (IY+d),A
            partial(self.shift, r, m, 23, 4, 0, 128, Yd, B),   # 0x20 SLA (IY+d),B
            partial(self.shift, r, m, 23, 4, 0, 128, Yd, C),   # 0x21 SLA (IY+d),C
            partial(self.shift, r, m, 23, 4, 0, 128, Yd, D),   # 0x22 SLA (IY+d),D
            partial(self.shift, r, m, 23, 4, 0, 128, Yd, E),   # 0x23 SLA (IY+d),E
            partial(self.shift, r, m, 23, 4, 0, 128, Yd, H),   # 0x24 SLA (IY+d),H
            partial(self.shift, r, m, 23, 4, 0, 128, Yd, L),   # 0x25 SLA (IY+d),L
            partial(self.shift, r, m, 23, 4, 0, 128, Yd),      # 0x26 SLA (IY+d)
            partial(self.shift, r, m, 23, 4, 0, 128, Yd, A),   # 0x27 SLA (IY+d),A
            partial(self.shift, r, m, 23, 4, 1, 1, Yd, B),     # 0x28 SRA (IY+d),B
            partial(self.shift, r, m, 23, 4, 1, 1, Yd, C),     # 0x29 SRA (IY+d),C
            partial(self.shift, r, m, 23, 4, 1, 1, Yd, D),     # 0x2A SRA (IY+d),D
            partial(self.shift, r, m, 23, 4, 1, 1, Yd, E),     # 0x2B SRA (IY+d),E
            partial(self.shift, r, m, 23, 4, 1, 1, Yd, H),     # 0x2C SRA (IY+d),H
            partial(self.shift, r, m, 23, 4, 1, 1, Yd, L),     # 0x2D SRA (IY+d),L
            partial(self.shift, r, m, 23, 4, 1, 1, Yd),        # 0x2E SRA (IY+d)
            partial(self.shift, r, m, 23, 4, 1, 1, Yd, A),     # 0x2F SRA (IY+d),A
            partial(self.shift, r, m, 23, 4, 2, 128, Yd, B),   # 0x30 SLL (IY+d),B
            partial(self.shift, r, m, 23, 4, 2, 128, Yd, C),   # 0x31 SLL (IY+d),C
            partial(self.shift, r, m, 23, 4, 2, 128, Yd, D),   # 0x32 SLL (IY+d),D
            partial(self.shift, r, m, 23, 4, 2, 128, Yd, E),   # 0x33 SLL (IY+d),E
            partial(self.shift, r, m, 23, 4, 2, 128, Yd, H),   # 0x34 SLL (IY+d),H
            partial(self.shift, r, m, 23, 4, 2, 128, Yd, L),   # 0x35 SLL (IY+d),L
            partial(self.shift, r, m, 23, 4, 2, 128, Yd),      # 0x36 SLL (IY+d)
            partial(self.shift, r, m, 23, 4, 2, 128, Yd, A),   # 0x37 SLL (IY+d),A
            partial(self.shift, r, m, 23, 4, 3, 1, Yd, B),     # 0x38 SRL (IY+d),B
            partial(self.shift, r, m, 23, 4, 3, 1, Yd, C),     # 0x39 SRL (IY+d),C
            partial(self.shift, r, m, 23, 4, 3, 1, Yd, D),     # 0x3A SRL (IY+d),D
            partial(self.shift, r, m, 23, 4, 3, 1, Yd, E),     # 0x3B SRL (IY+d),E
            partial(self.shift, r, m, 23, 4, 3, 1, Yd, H),     # 0x3C SRL (IY+d),H
            partial(self.shift, r, m, 23, 4, 3, 1, Yd, L),     # 0x3D SRL (IY+d),L
            partial(self.shift, r, m, 23, 4, 3, 1, Yd),        # 0x3E SRL (IY+d)
            partial(self.shift, r, m, 23, 4, 3, 1, Yd, A),     # 0x3F SRL (IY+d),A
