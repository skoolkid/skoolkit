...
#define REG(r) ((unsigned)reg[r])
#define LD(r, v) reg[r] = v
#define PEEK(a) (mem ? mem[a] : self->mem128[(a) / 0x4000][(a) % 0x4000])
#define POKE(a, v) if (mem) mem[a] = v; else self->mem128[(a) / 0x4000][(a) % 0x4000] = v
#define INC_R(i) LD(R, (REG(R) & 0x80) + ((REG(R) + (i)) & 0x7F))
#define TIME reg[T]
#ifdef CONTENTION
...
#define INC_PC(i) LD(PC, ADDR(REG(PC) + (i)))
...
typedef unsigned (*read_port_f)(CSimulatorObject* self, unsigned port);
...
typedef struct CSimulatorObject {
    ...
    PyObject* tracer;
    unsigned long long* tracer_state;
    unsigned long long* tape_edges;
    unsigned long long max_index;
    unsigned in_min_addr;
    tsl_accelerator** accelerators;
    unsigned num_accs;
    unsigned tsl_misses;
...
static int accept_interrupt(CSimulatorObject* self, unsigned prev_pc) {
    ...
    byte opcode = PEEK(prev_pc);
    unsigned pc = REG(PC);
    if (opcode == 0xFB || ((opcode == 0xDD || opcode == 0xFD) && prev_pc == ADDR(pc - 1))) {
        return 0;
    }
    unsigned iaddr = 56;
    if (REG(IM) == 2) {
        unsigned vaddr = 255 + 256 * REG(I);
        iaddr = PEEK(vaddr) + 256 * PEEK(ADDR(vaddr + 1));
        TIME += 19;
    } else {
        TIME += 13;
    }
    unsigned sp = ADDR(REG(SP) - 2);
    LD(SP, sp);
    if (sp > 0x3FFF) {
        POKE(sp, pc % 256);
    }
...
static void af_xy(CSimulatorObject* self, void* lookup, int args[]) {
    ...
    unsigned d = PEEK(ADDR(REG(PC) + 2));
    unsigned addr = ADDR(xy + (d < 128 ? d : d - 256));
#ifdef CONTENTION
    CONTEND {
        unsigned pc = REG(PC);
...
static void call(CSimulatorObject* self, void* lookup, int args[]) {
    ...
    if (c_and && (REG(F) & c_and) == c_val) {
        ...
        byte* mem = self->memory;
        unsigned pc = REG(PC);
        LD(PC, PEEK(ADDR(pc + 1)) + 256 * PEEK(ADDR(pc + 2)));
        unsigned ret_addr = ADDR(pc + 3);
        unsigned sp = ADDR(REG(SP) - 2);
        LD(SP, sp);
        if (sp > 0x3FFF) {
            POKE(sp, ret_addr % 256);
        }
        sp = ADDR(sp + 1);
        if (sp > 0x3FFF) {
            POKE(sp, ret_addr / 256);
...
static void cpi(CSimulatorObject* self, void* lookup, int args[]) {
    ...
    if (repeat && cp && bc) {
        ...
        int n = cp - hf;
        LD(F, f + (cp == 0) * 0x40 + (n & 0x02) * 16 + (n & 0x08) + (bc > 0) * 0x04); // .Z5.3P..
#ifdef CONTENTION
        CONTEND {
            unsigned pc = REG(PC);
            hl = ADDR(hl - inc);
...
#endif
    INC_T(4);
    if (REG(IFF) && (TIME % self->frame_duration) < self->int_active) {
        INC_PC(1);
        LD(HALT, 0);
    } else {
        LD(HALT, 1);
...
static void in_a(CSimulatorObject* self, void* lookup, int args[]) {
    ...
    if (self->in_a_n_tracer) {
        ...
        if (self->read_port) {
            ...
            PyObject* rv = m_args ? PyObject_Call(self->in_a_n_tracer, m_args, NULL) : NULL;
            Py_XDECREF(m_args);
            if (rv) {
...
static void in_c(CSimulatorObject* self, void* lookup, int args[]) {
    ...
    if (self->in_r_c_tracer) {
        ...
        if (self->read_port) {
            ...
            PyObject* m_args = Py_BuildValue("(OI)", self->registers_obj, port);
            PyObject* rv = m_args ? PyObject_Call(self->in_r_c_tracer, m_args, NULL) : NULL;
            Py_XDECREF(m_args);
            if (rv) {
                value = PyLong_AsLong(rv);
                Py_DECREF(rv);
...
#endif
    if (rl == SP) {
        LD(SP, ADDR(REG(SP) + inc));
    } else {
        unsigned value = ADDR(REG(rl) + 256 * REG(rh) + inc);
        LD(rh, value / 256);
        LD(rl, value % 256);
    }
...
static void ini(CSimulatorObject* self, void* lookup, int args[]) {
    ...
    if (self->ini_tracer) {
        ...
        if (self->read_port) {
            ...
            PyObject* m_args = Py_BuildValue("(OI)", self->registers_obj, port);
            PyObject* rv = m_args ? PyObject_Call(self->ini_tracer, m_args, NULL) : NULL;
            Py_XDECREF(m_args);
            if (rv) {
...
static void jp(CSimulatorObject* self, void* lookup, int args[]) {
    ...
    if ((REG(F) & c_and) == c_val) {
        byte* mem = self->memory;
        LD(PC, PEEK(ADDR(REG(PC) + 1)) + 256 * PEEK(ADDR(REG(PC) + 2)));
    } else {
        INC_PC(3);
    }
...
static void ld_a_ir(CSimulatorObject* self, void* lookup, int args[]) {
    ...
    INC_T(9);
    if (REG(IFF) && (TIME % self->frame_duration) < self->int_active) {
        LD(F, (a & 0xA8) + (a == 0) * 0x40 + (REG(F) & 1));
    } else {
        LD(F, (a & 0xA8) + (a == 0) * 0x40 + REG(IFF) * 0x04 + (REG(F) & 1));
    }
    INC_PC(2);
}
...
static void ld_r_xy(CSimulatorObject* self, void* lookup, int args[]) {
    ...
    unsigned xy = REG(xyl) + 256 * REG(xyh);
    unsigned offset = PEEK(ADDR(REG(PC) + 2));
    unsigned addr = ADDR(xy + (offset < 128 ? offset : offset - 256));
#ifdef CONTENTION
    CONTEND {
        unsigned pc = REG(PC);
        unsigned pc2 = ADDR(pc + 2);
...
#endif
    if (rl == SP) {
        LD(SP, PEEK(ADDR(REG(PC) + 1)) + 256 * PEEK(ADDR(REG(PC) + 2)));
    } else {
        LD(rl, PEEK(ADDR(REG(PC) + size - 2)));
        LD(rh, PEEK(ADDR(REG(PC) + size - 1)));
    }
...
static void ld_m_a(CSimulatorObject* self, void* lookup, int args[]) {
    ...
    unsigned addr = PEEK(ADDR(REG(PC) + 1)) + 256 * PEEK(ADDR(REG(PC) + 2));
#ifdef CONTENTION
    CONTEND {
        unsigned pc = REG(PC);
...
static void ld_rr_mm(CSimulatorObject* self, void* lookup, int args[]) {
    ...
    unsigned addr = PEEK(ADDR(REG(PC) + size - 2)) + 256 * PEEK(ADDR(REG(PC) + size - 1));
#ifdef CONTENTION
    CONTEND {
        unsigned pc = REG(PC);
        if (size == 3) {
...
static void ldi(CSimulatorObject* self, void* lookup, int args[]) {
    ...
    if (repeat && bc) {
        ...
        LD(F, (REG(F) & 0xC1) + (n & 0x02) * 16 + (n & 0x08) + (bc > 0) * 0x04);
#ifdef CONTENTION
        CONTEND {
            unsigned pc = REG(PC);
            hl = ADDR(hl - inc);
...
static void out_a(CSimulatorObject* self, void* lookup, int args[]) {
    ...
    if (self->out_tracer) {
        ...
        PyObject* rv = m_args ? PyObject_Call(self->out_tracer, m_args, NULL) : NULL;
        Py_XDECREF(m_args);
        if (rv == NULL) {
            return;
        }
...
static void rld(CSimulatorObject* self, void* lookup, int args[]) {
    ...
    byte at_hl = PEEK(hl);
    if (hl > 0x3FFF) {
        POKE(hl, ((at_hl * 16) % 256) + (a % 16));
    }
    LD(A, (a & 240) + at_hl / 16);
    LD(F, SZ53P[REG(A)] + (REG(F) & 1));
...
static void rst(CSimulatorObject* self, void* lookup, int args[]) {
    ...
    unsigned ret_addr = ADDR(REG(PC) + 1);
    unsigned sp = ADDR(REG(SP) - 2);
    LD(SP, sp);
    if (sp > 0x3FFF) {
        POKE(sp, ret_addr % 256);
...
static PyObject* CSimulator_exec_with_cb(CSimulatorObject* self, PyObject* args, PyObject* kwds) {
    ...
    while (1) {
        ...
        if (opcode == 0xD7) {
            ...
            PyObject* rv = arg ? PyObject_CallOneArg(rst16_cb, arg) : NULL;
            Py_XDECREF(arg);
            if (rv == NULL) {
                return NULL;
            }
...
static PyObject* CSimulator_exec_frame(CSimulatorObject* self, PyObject* args, PyObject* kwds) {
    ...
    while (1) {
        ...
        if (exec_map != Py_None) {
            ...
            int rv = addr ? PySet_Add(exec_map, addr) : -1;
            Py_XDECREF(addr);
            if (rv == -1) {
                return NULL;
            }
        ...
        if (trace != Py_None) {
            ...
            PyObject* rv = m_args ? PyObject_Call(trace, m_args, NULL) : NULL;
            Py_XDECREF(m_args);
            if (rv == NULL) {
                return NULL;
            }
...
static PyObject* CSimulator_trace(CSimulatorObject* self, PyObject* args, PyObject* kwds) {
    ...
    while (1) {
        ...
        if (disassembling) {
            ...
            i = arg ? PyObject_CallOneArg(disassemble, arg) : NULL;
            Py_XDECREF(arg);
            if (i == NULL) {
                return NULL;
            }
        ...
        if (REG(IFF) && (TIME % frame_duration) < int_active) {
            if (accept_interrupt(self, pc) && PyList_Size(keys)) {
                ...
                if (!PyObject_IsTrue(k)) {
                    ...
                    PyObject* p = arg ? PyObject_CallOneArg(pop, arg) : NULL;
                    Py_XDECREF(arg);
                    if (p == NULL) {
                        ok = 0;
                        break;
...
static int get_tsl_accelerators(PyObject* accelerators, tsl_accelerator* accs) {
    ...
    while ((item = PyIter_Next(iter)) != NULL) {
        ...
        PyObject* counter_obj = PyObject_GetAttrString(item, "counter");
        PyObject* inc_obj = PyObject_GetAttrString(item, "inc");
        ...
        PyObject* ear_obj = PyObject_GetAttrString(item, "ear");
        PyObject* ear_mask_obj = PyObject_GetAttrString(item, "ear_mask");
        PyObject* polarity_obj = PyObject_GetAttrString(item, "polarity");
        if (name == NULL || code_obj == NULL || c0_obj == NULL || c1_obj == NULL || counter_obj == NULL || inc_obj == NULL ||
            ...
            if (accelerator->code) {
                for (int i = 0; i < code_len; i++) {
                    ...
                    if (b == NULL) {
                        ...
                        accelerator->code[i] = PyLong_Check(b) ? PyLong_AsLong(b) : 256;
                    }
                }
            } else {
                PyErr_NoMemory();
...
static unsigned read_port(CSimulatorObject* self, unsigned port) {
    unsigned long long* reg = self->registers;
    ...
    if ((port & 0xFF) == 0xFE) {
        ...
        if (pc >= self->in_min_addr || (pc >= 0x0562 && pc <= 0x05F1 && self->out7ffd & 0x10)) {
            ...
            if (self->tracer_state[7] && !self->tracer_state[2]) {
                ...
                PyObject* block_index_obj = blocks_obj ? PyObject_GetAttrString(self->tracer, "block_index") : NULL;
                PyObject* block = block_index_obj ? PyList_GetItem(blocks_obj, PyLong_AsLong(block_index_obj)) : NULL;
                PyObject* data = block ? PyObject_GetAttrString(block, "data") : NULL;
                Py_ssize_t len = PyBytes_Check(data) ? PyBytes_Size(data) : 0;
                Py_XDECREF(data);
                Py_XDECREF(block_index_obj);
                Py_XDECREF(blocks_obj);
                if (len) {
                    PyObject* s = PyUnicode_FromFormat("Data (%u bytes)", len);
                    if (s) {
                        Py_XDECREF(PyObject_CallOneArg(self->write_line, s));
                        Py_DECREF(s);
                    }
                }
            } else if (index == self->max_index) {
                Py_XDECREF(PyObject_CallMethod(self->tracer, "stop_tape", "(K)", TIME));
            } else if (self->accelerators && self->tracer_state[4] && REG(IFF) == 0 && index < self->tracer_state[3] - 1) {
                byte* mem = self->memory;
                unsigned loops = 0;
                unsigned tsl_miss = 1;
                for (unsigned k = 0; k < self->num_accs; k++) {
                    tsl_accelerator* acc = self->accelerators[k];
                    int match = 1;
                    int i = 0;
                    for (int j = 0 - acc->c0; j < acc->c1; j++) {
                        unsigned c = acc->code[i++];
                        if (c < 256 && c != PEEK(ADDR(pc + j))) {
                            match = 0;
                            break;
                        }
                    }
                    if (match) {
                        ...
                        if (k) {
                            ...
                            tsl_accelerator* first = self->accelerators[0];
                            self->accelerators[0] = acc;
                            self->accelerators[k] = first;
                        }
                        break;
                    }
                }
                self->tsl_misses += tsl_miss;
            }
            return (index & 1) ? 0xFF : 0xBF;
        }
    } else if ((port & 0xC002) == 0xC000) {
        PyObject* outfffd_obj = PyObject_GetAttrString(self->tracer, "outfffd");
        unsigned ay_reg = outfffd_obj ? PyLong_AsLong(outfffd_obj) : 0xFF;
        ...
        if (ay_reg < 16) {
            ...
            PyObject* value = ay ? PyList_GetItem(ay, ay_reg) : NULL;
            Py_XDECREF(ay);
            return value ? PyLong_AsLong(value) : 0xFF;
        }
    }
...
static PyObject* CSimulator_load(CSimulatorObject* self, PyObject* args, PyObject* kwds) {
    ...
    while (1) {
        ...
        pc = REG(PC);
        unsigned long long end_of_tape = self->tracer_state[2];
        ...
        if (pc == 0x0556 && self->out7ffd & 0x10 && fast_load) {
            ...
            if (did_fast_load) {
                unsigned long long block_max_index = self->tracer_state[3];
                self->tracer_state[1] = block_max_index;
                if (block_max_index == self->max_index) {
...
cleanup:
    ...
    if (rv && list_accelerators && accelerators && accs && num_accs) {
                    ...
                    for (int i = 0; i < num_accs; i++) {
                        ...
                        if (acc->name == name) {
                            if (acc->hits) {
                                ...
                                if (PyObject_SetAttrString(item, "hits", hits) == -1) {
                                    ok = 0;
                                }
                                Py_XDECREF(hits);
                            }
        ...
        PyObject* tsl_misses = ok ? PyLong_FromLong(self->tsl_misses) : NULL;
        PyObject* dec_a_jr_hits = tsl_misses ? PyLong_FromLong(dec_a_accelerator.args[0]) : NULL;
        PyObject* dec_a_jp_hits = dec_a_jr_hits ? PyLong_FromLong(dec_a_accelerator.args[1]) : NULL;
        PyObject* dec_a_misses = dec_a_jp_hits ? PyLong_FromLong(dec_a_accelerator.args[2]) : NULL;
        ok = dec_a_misses != NULL;
        if (!ok ||
            PyObject_SetAttrString(self->tracer, "tsl_misses", tsl_misses) == -1 ||
            PyObject_SetAttrString(self->tracer, "dec_a_jr_hits", dec_a_jr_hits) == -1 ||
