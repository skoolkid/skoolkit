...
static int accept_interrupt(CSimulatorObject* self, unsigned prev_pc) {
    ...
    if (REG(IM) == 2) {
        ...
        iaddr = PEEK(vaddr) + 256 * PEEK((vaddr + 1) % 65536);
        TIME += 19;
    } else {
        TIME += 13;
    }
...
static void call(CSimulatorObject* self, void* lookup, int args[]) {
    ...
    if (c_and && (REG(F) & c_and) == c_val) {
        ...
        byte* mem = self->memory;
        unsigned pc = REG(PC);
        LD(PC, PEEK((pc + 1) % 65536) + 256 * PEEK((pc + 2) % 65536));
...
static void djnz(CSimulatorObject* self, void* lookup, int args[]) {
    ...
    if (b) {
        ...
        unsigned pc = REG(PC);
        byte* mem = self->memory;
        int offset = PEEK((pc + 1) % 65536);
        LD(PC, (pc + (offset < 128 ? offset + 2 : offset - 254)) % 65536);
    } else {
#ifdef CONTENTION
        CONTEND {
            unsigned pc = REG(PC);
...
static void in_a(CSimulatorObject* self, void* lookup, int args[]) {
    ...
    if (self->in_a_n_tracer) {
        byte* mem = self->memory;
        unsigned port = PEEK((REG(PC) + 1) % 65536) + 256 * REG(A);
...
static void jp(CSimulatorObject* self, void* lookup, int args[]) {
    ...
    if ((REG(F) & c_and) == c_val) {
        byte* mem = self->memory;
        LD(PC, PEEK((REG(PC) + 1) % 65536) + 256 * PEEK((REG(PC) + 2) % 65536));
    } else {
        INC_PC(3);
    }
...
#endif
        byte* mem = self->memory;
        byte offset = PEEK((REG(PC) + 1) % 65536);
...
static void ld_r_xy(CSimulatorObject* self, void* lookup, int args[]) {
    ...
    unsigned offset = PEEK((REG(PC) + 2) % 65536);
...
#endif
    if (rl == SP) {
        LD(SP, PEEK((REG(PC) + 1) % 65536) + 256 * PEEK((REG(PC) + 2) % 65536));
    } else {
        LD(rl, PEEK((REG(PC) + size - 2) % 65536));
        LD(rh, PEEK((REG(PC) + size - 1) % 65536));
    }
...
#endif
    LD(A, PEEK(PEEK((REG(PC) + 1) % 65536) + 256 * PEEK((REG(PC) + 2) % 65536)));
...
static void ld_m_a(CSimulatorObject* self, void* lookup, int args[]) {
    ...
    unsigned addr = PEEK((REG(PC) + 1) % 65536) + 256 * PEEK((REG(PC) + 2) % 65536);
#ifdef CONTENTION
    CONTEND {
        unsigned pc = REG(PC);
...
static void ld_rr_mm(CSimulatorObject* self, void* lookup, int args[]) {
    ...
    unsigned addr = PEEK((REG(PC) + size - 2) % 65536) + 256 * PEEK((REG(PC) + size - 1) % 65536);
#ifdef CONTENTION
    CONTEND {
        unsigned pc = REG(PC);
        if (size == 3) {
...
#endif
    if (rl == SP) {
        LD(SP, PEEK(addr) + 256 * PEEK((addr + 1) % 65536));
    } else {
        LD(rl, PEEK(addr));
        LD(rh, PEEK((addr + 1) % 65536));
    }
...
static void ret(CSimulatorObject* self, void* lookup, int args[]) {
    ...
    if (c_and) {
        if ((REG(F) & c_and) == c_val) {
            ...
            INC_T(11);
            LD(SP, (sp + 2) % 65536);
            LD(PC, PEEK(sp) + 256 * PEEK((sp + 1) % 65536));
        }
    } else {
        unsigned sp = REG(SP);
#ifdef CONTENTION
    ...
    while (1) {
        ...
        if (!opcode_func->func) {
            ...
            switch (opcode) {
                ...
                case 0xDD:
                    if (opcode2 == 0xCB) {
                        opcode_func = &after_DDCB[PEEK((pc + 3) % 65536)];
                    } else {
                ...
                case 0xFD:
                    if (opcode2 == 0xCB) {
                        opcode_func = &after_FDCB[PEEK((pc + 3) % 65536)];
                    } else {
...
static PyObject* CSimulator_exec_with_cb(CSimulatorObject* self, PyObject* args, PyObject* kwds) {
    ...
    while (1) {
        ...
        OpcodeFunction* opcode_func = &opcodes[opcode];
        if (!opcode_func->func) {
            byte opcode2 = PEEK((REG(PC) + 1) % 65536);
            switch (opcode) {
                case 0xCB:
                    opcode_func = &after_CB[opcode2];
                    break;
                ...
                case 0xDD:
                    if (opcode2 == 0xCB) {
                        opcode_func = &after_DDCB[PEEK((REG(PC) + 3) % 65536)];
                    } else {
                        opcode_func = &after_DD[opcode2];
                    }
                    break;
                case 0xFD:
                    if (opcode2 == 0xCB) {
                        opcode_func = &after_FDCB[PEEK((REG(PC) + 3) % 65536)];
                    } else {
                        opcode_func = &after_FD[opcode2];
                    }
                    break;
...
static void dec_a_jr(CSimulatorObject* self, void* lookup, int args[]) {
    ...
    if (REG(IFF) == 0 && a && PEEK(pc1) == 0x20 && PEEK((pc1 + 1) % 65536) == 0xFD) {
        LD(A, 0);
        LD(F, 0x42 + (REG(F) & 1));
        INC_R(a * 2);
        INC_T(16 * a - 5);
...
static void dec_a_jp(CSimulatorObject* self, void* lookup, int args[]) {
    ...
    byte a = REG(A);
    unsigned pc = REG(PC);
    if (REG(IFF) == 0 && a && PEEK((pc + 1) % 65536) == 0xC2 && PEEK((pc + 2) % 65536) == pc % 256 && PEEK((pc + 3) % 65536) == pc / 256) {
        LD(A, 0);
        LD(F, 0x42 + (REG(F) & 1));
        INC_R(a * 2);
        INC_T(14 * a);
...
static void dec_a_list(CSimulatorObject* self, void* lookup, int args[]) {
    ...
    unsigned pc = REG(PC);
    if (REG(IFF) == 0) {
        if (PEEK((pc + 1) % 65536) == 0x20 && PEEK((pc + 2) % 65536) == 0xFD) {
            args[0]++;
            dec_a_jr(self, lookup, args);
            return;
        }
        if (PEEK((pc + 1) % 65536) == 0xC2 && PEEK((pc + 2) % 65536) == pc % 256 && PEEK((pc + 3) % 65536) == pc / 256) {
            args[1]++;
        ...
        for (int k = 0; k < num_accs; k++) {
            ...
            if (match) {
                if ((REG(C) & acc->ear_mask) == ((state[1] - acc->polarity) & 1) * acc->ear_mask) {
                    int delta = (int)(state[0] - TIME - acc->in_time);
                    if (delta > 0) {
