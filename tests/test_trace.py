...
class TraceTest(SkoolKitTestCase):
    ...
    def test_default_option_values(self):
        ...
        self.assertEqual(options.ay_mode, 'MONO')
    ...
    def test_z80_128k(self):
        ...
        z80file = self.write_z80(ram, registers, machine_id=4, out7ffd=18, pages=pages)
        exp_output = """
    ...
    def test_z80_plus2(self):
        ...
        z80file = self.write_z80(ram, registers, machine_id=4, modify=True, out7ffd=18, pages=pages)
        exp_output = """
            $6000 OUT (C),A        A=01  F=00000010  BC=7FFD  DE=0506  HL=0708  IX=0B0C IY=090A
                                   A'=0F F'=00010000 BC'=1112 DE'=1314 HL'=1516 SP=FFFF IR=0D10
            Stopped at $6002
    ...
    def test_interrupt_routine_executed_twice_while_int_is_active_128k(self):
        ...
        z80file = self.write_z80(ram, registers, machine_id=4)
        output, error = self.run_trace(f'-S {stop} -v {z80file}')
        self.assertEqual(error, '')
        exp_output = """
            $7FF9 NOP
    ...
    def test_option_audio_with_ay(self):
        ...
        z80file = self.write_z80(ram, {'PC': start}, machine_id=4)
        exp_output = """
            Stopped at $8016
    ...
    def test_option_ay_mode_MONO(self):
        ...
        outfile = 'out.wav'
        output, error = self.run_trace(f'-S {stop} --ay --ay-mode MONO {infile} {outfile}')
        exp_output = f"""
            Stopped at ${stop:04X}
            Wrote {outfile}
        """
    ...
    def test_option_ay_mode_ABC(self):
        ...
        infile = self.write_z80(ram, {'PC': start}, machine_id=4)
        outfile = 'out.wav'
        output, error = self.run_trace(f'-S {stop} --ay --ay-mode ABC {infile} {outfile}')
        exp_output = f"""
            Stopped at ${stop:04X}
            Wrote {outfile}
        """
    ...
    def test_option_ay_mode_ACB(self):
        ...
        infile = self.write_z80(ram, {'PC': start}, machine_id=4)
        outfile = 'out.wav'
        output, error = self.run_trace(f'-S {stop} --ay --ay-mode ACB {infile} {outfile}')
        exp_output = f"""
            Stopped at ${stop:04X}
            Wrote {outfile}
        """
    ...
    def test_option_reg_with_snapshot(self):
        ...
        z80file = self.write_z80(ram, z80reg)
        registers = {
            'A': 0x50,
            'F': 0b01000001,
            'BC': 0x1234,
    ...
    def test_option_start(self):
        ram = [0] * 49152
        z80file = self.write_z80(ram, {'PC': 16384})
    ...
    def test_option_state_with_48k_snapshot(self):
        ...
        ram = [0] * 49152
        z80file = self.write_z80(ram, header=header)
        state = ('border=4', 'fe=128', 'iff=0', 'im=2', 'tstates=40000')
        state_options = ' '.join(f'--state {spec}' for spec in state)
    ...
    def test_option_volume_beeper(self):
        data = (
        ...
        stop = start + len(data)
        volume = 50
        output, error = self.run_trace(f'-n -o {start} -S {stop} --volume {volume} {infile} {outfile}')
        exp_output = f"""
            Stopped at ${stop:04X}
            Wrote {outfile}
        """
        ...
        self.assertEqual(audio_writer.options.volume, volume)
    ...
    @patch.object(trace, 'get_audio_writer', MockAudioWriter)
    def test_option_volume_beeper_over_100(self):
        data = (
            0x0E, 0x0A,             # $8000 LD C,$0A
            0xD3, 0xFE,             # $8002 OUT ($FE),A
            0xEE, 0x10,             # $8004 XOR $10
        ...
        start = 32768
        stop = start + len(data)
        output, error = self.run_trace(f'-n -o {start} -S {stop} --volume 150 {infile} {outfile}')
        exp_output = f"""
            Stopped at ${stop:04X}
            Wrote {outfile}
        """
        ...
        self.assertEqual(audio_writer.fname, outfile)
        self.assertEqual(exp_delays, audio_writer.delays)
        self.assertEqual(audio_writer.options.volume, 100)

    @patch.object(trace, 'get_audio_writer', MockAudioWriter)
    def test_option_volume_beeper_under_0(self):
        data = (
            0x0E, 0x0A,             # $8000 LD C,$0A
            0xD3, 0xFE,             # $8002 OUT ($FE),A
            0xEE, 0x10,             # $8004 XOR $10
        ...
        start = 32768
        stop = start + len(data)
        output, error = self.run_trace(f'-n -o {start} -S {stop} --volume -10 {infile} {outfile}')
        exp_output = f"""
            Stopped at ${stop:04X}
            Wrote {outfile}
        """
    ...
    def test_write_multiple_files(self):
        data = [
            0x21, 0x00, 0x40,       # $8000 LD HL,$4000
            0xD3, 0xFE,             # $8003 OUT ($FE),A
            0x74,                   # $8005 LD (HL),H
            0xEE, 0x10,             # $8006 XOR $10
            0x24,                   # $8008 INC H
            0xCB, 0x5C,             # $8009 BIT 3,H
            0x28, 0xF6,             # $800B JR Z,$8003
        ]
        infile = self.write_bin_file(data, suffix='.bin')
        start = 32768
        stop = start + len(data)
        output, error = self.run_trace(f'-n -o {start} -S {stop} {infile} out.png out.wav out.z80')
        exp_output = f"""
            Stopped at ${stop:04X}
            Wrote out.png
            Wrote out.wav
        ...
        self.assertEqual(len(image_writer.frames), 1)
        udgs = image_writer.frames[0].udgs
        self.assertEqual(len(udgs), 24)
        self.assertEqual(len(udgs[0]), 32)
        self.assertEqual([64, 65, 66, 67, 68, 69, 70, 71], list(udgs[0][0].data))
        self.assertEqual(image_writer.fname, 'out.png')

        self.assertEqual([49] * 7, audio_writer.delays)
        self.assertEqual(audio_writer.fname, 'out.wav')
    ...
    def test_write_png_128k(self):
        ...
        stop = start + len(data)
        ram[start - 0x4000:stop - 0x4000] = data
        infile = self.write_z80(ram, machine_id=4)
        outfile = 'out.png'
    ...
    def test_custom_screen(self):
        ...
        stop = start + len(data)
        ram[start - 0x4000:stop - 0x4000] = data
        infile = self.write_z80(ram, {'PC': start, 'tstates': 69850})
    ...
    def test_write_z80_plus2(self):
        ...
        infile = self.write_z80(ram, machine_id=4, modify=True)
        outfile = 'out.z80'
        output, error = self.run_trace(f'-s {start} -S {stop} {infile} {outfile}')
        exp_output = f"""
            Stopped at ${stop:04X}
    ...
    def test_ay_tracing_from_z80(self):
        ...
        z80file = self.write_z80(ram, machine_id=4, ay=ay)
    ...
    def test_bit_5_of_output_to_port_0x7ffd(self):
        ...
        stop = start + len(code)
        z80file = self.write_z80(ram, machine_id=4)
        stop = start + len(code)
