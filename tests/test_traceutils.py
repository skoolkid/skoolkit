...
class DisassembleTest(SkoolKitTestCase):
    def _test_instructions(self, opcodes, addr):
        memory = [0] * 65536
        for h, (exp_operation, exp_size) in opcodes.items():
            ...
            for i in range(0, len(h), 2):
                ...
                a = (a + 1) % 65536
            operation, size = disassemble(memory, addr)
    ...
    def test_all_instructions(self):
        self._test_instructions(OPCODES, 0)

    def test_instructions_at_65535(self):
        opcodes = {h: (op, size) for h, (op, size) in OPCODES.items() if size > 1}
        opcodes.update({
    ...
    def test_instructions_at_65534(self):
        opcodes = {h: (op, size) for h, (op, size) in OPCODES.items() if size > 2}
        self._test_instructions(opcodes, 65534)

    def test_instructions_at_65533(self):
        opcodes = {h: (op, size) for h, (op, size) in OPCODES.items() if size > 3}
